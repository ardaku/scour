  ...
  if (matched) {
    ...
    penalty =
      penalty < MAX_LEADING_LETTER_PENALTY
        ...
        : penalty;
    outScore += penalty;
    console.log("Leading letter penalty of", penalty, " - Now", outScore);

    //Apply unmatched penalty
    console.log([str.length, nextMatch]);
    const unmatched = (UNMATCHED_LETTER_PENALTY * (str.length - nextMatch));
    outScore += unmatched;
    console.log("Unmatched letter penalty of", unmatched, " - Now", outScore);
    ...
    for (let i = 0; i < nextMatch; i++) {
      ...
      if (i > 0) {
        ...
        if (currIdx == prevIdx + 1) {
          ...
          console.log("Applied sequential bonus", SEQUENTIAL_BONUS, " - Now", outScore);
        }
      }
      ...
      if (currIdx > 0) {
        ...
        if (
          ...
          console.log("Applied camel bonus", " - Now", outScore);
        }
        const isNeighbourSeparator = neighbor == "_" || neighbor == " ";
        if (isNeighbourSeparator) {
          outScore += SEPARATOR_BONUS;
          console.log("Applied separator bonus", " - Now", outScore);
        }
      } else {
        // First letter
        outScore += FIRST_LETTER_BONUS;
        console.log("Applied first letter bonus", " - Now", outScore);
      }
    }
